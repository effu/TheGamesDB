	...
	<?php endif;?>
		...
		<?php
$messageQuery = $database->query("SELECT m.id, m.to, m.subject, m.message, m.timestamp, m.status, u.username AS fromname FROM messages AS m, users AS u WHERE m.id = $messageid AND m.from = u.id;");
$message = $messageQuery->fetch(PDO::FETCH_OBJ);
if ($message->to == $user->id) {
    $database->query(" UPDATE messages SET status = 'read' WHERE id = '$messageid' ");
    ?>

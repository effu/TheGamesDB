...
if ($result === false) {
    ...
    if (!empty($result["matches"])) {
        foreach ($result["matches"] as $id => $junk) {
            ...
            $subresult = $database->query($subquery) or die('Query failed: ' . mysql_error());
            $db = $subresult->fetch(PDO::FETCH_OBJ);
            if (!$db->seriesid) {
                continue;
            }
            ...
            $subresult = $database->query($subquery) or die('Query failed: ' . mysql_error());
            if ($subdb = $subresult->fetch(PDO::FETCH_OBJ)) {

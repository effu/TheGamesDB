...
function processFanart($gameID)
{
    ...
    $faResult = $database->query(" SELECT filename FROM banners WHERE keyvalue = $gameID AND keytype = 'fanart' ORDER BY filename ASC ");

    ## Process each fanart row incrementally
    while ($faRow = $faResult->fetch(PDO::FETCH_ASSOC)) {
...
{
    ## Select all fanart rows for the requested game id
    $ssResult = $database->query(" SELECT filename FROM banners WHERE keyvalue = $gameID AND keytype = 'screenshot' ORDER BY filename ASC ");

    ## Process each fanart row incrementally
    while ($ssRow = $ssResult->fetch(PDO::FETCH_ASSOC)) {
        ## Construct file names
...
function processBoxart($gameID)
{
    ...
    $baResult = $database->query(" SELECT filename FROM banners WHERE keyvalue = $gameID AND keytype = 'boxart' ORDER BY filename ASC ");

    ## Process each boxart row incrementally
    while ($baRow = $baResult->fetch(PDO::FETCH_ASSOC)) {
        ## Construct file names
...
{
    ## Select all boxart rows for the requested game id
    $banResult = $database->query(" SELECT filename FROM banners WHERE keyvalue = $gameID AND keytype = 'series' ORDER BY filename ASC ");

    ## Process each boxart row incrementally
    while ($banRow = $banResult->fetch(PDO::FETCH_ASSOC)) {
        ## Construct file names
...
{
    ## Select all boxart rows for the requested game id
    $clResult = $database->query(" SELECT filename, resolution FROM banners WHERE keyvalue = $gameID AND keytype = 'clearlogo' LIMIT 1 ");

    ## Process each boxart row incrementally
    while ($clRow = $clResult->fetch(PDO::FETCH_ASSOC)) {
        ## Construct file names
...
if (isset($id) && !empty($id)) {
    ...
    if (isset($platform) && !empty($platform)) {
        $platformResult = $database->query(" SELECT id FROM platforms WHERE name = '$platform' LIMIT 1 ");
        if (count($platformResult) != 0) {
            $platformRow = $platformResult->fetch(PDO::FETCH_ASSOC);
...
while ($obj = $result->fetch(PDO::FETCH_OBJ)) {
    ...
    ##On Other Platforms
    $similarResult = $database->query("SELECT g.id, g.platform FROM games as g, platforms as p WHERE g.GameTitle = \"$baseObj->GameTitle\" AND g.Platform = p.id AND g.Platform != '$baseObj->PlatformId' ORDER BY p.name");

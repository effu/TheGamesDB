...
?>
	...
	<?php
$recentResult = $database->query(" SELECT g.*, p.name, p.icon, AVG(r.rating) as toprating, COUNT(r.rating) as countrating FROM games AS g, platforms AS p, ratings AS r WHERE r.itemid = g.id AND r.itemtype = 'game' AND g.platform = p.id GROUP BY g.GameTitle ORDER BY toprating DESC, countrating DESC LIMIT 50 ");
$count = 1;
$recent = $recentResult->fetch(PDO::FETCH_OBJ)
...
?>
		<div style=" width: 90%; padding: 16px; margin: 10px auto 20px auto; border-radius: 4px; border: 1px solid #4f4f4f; background-color: #333;">
			...
			<?php
if ($boxartResult = $database->query(" SELECT b.filename FROM banners as b WHERE b.keyvalue = '$recent->id' AND b.filename LIKE '%boxart%front%' LIMIT 1 ")) {
...
?>
			...
			<div>
				<p>
				<?php
$boxartQuery = $database->query("SELECT keyvalue FROM banners WHERE banners.keyvalue = '$recent->id' AND banners.filename LIKE '%front%' LIMIT 1");
$boxartResult = count($boxartQuery);

$fanartQuery = $database->query("SELECT keyvalue FROM banners WHERE banners.keyvalue = '$recent->id' AND keytype = 'fanart' LIMIT 1");
$fanartResult = count($fanartQuery);

$bannerQuery = $database->query("SELECT keyvalue FROM banners WHERE banners.keyvalue = '$recent->id' AND keytype = 'series' LIMIT 1");
$bannerResult = count($bannerQuery);

$screenQuery = $database->query("SELECT keyvalue FROM banners WHERE banners.keyvalue = '$recent->id' AND keytype = 'screenshot' LIMIT 1");
$screenResult = count($screenQuery);
?>

...
if ($loggedin == 1) {
    ?>
		...
		<?php
$messagesQuery = $database->query("SELECT m.id, m.subject, m.status, m.timestamp, u.username AS fromname FROM messages AS m, users AS u WHERE m.to = '$user->id' AND m.from = u.id");
    ?>
...
if (count($messagesQuery) != 0) {
        while ($messages = $messagesQuery->fetch(PDO::FETCH_OBJ)) {
            if ($rowcolor == "#bbb") {
                $rowcolor = "#999";
            } else {
                $rowcolor = "#bbb";

...
if (count($result) != 0) {
	...
	<form id="editPlatformForm" name="editPlatformForm" action="<?=$baseurl?>/platform-edit/<?=$platform->id?>/" method="post" onsubmit="">
        ...
        if ($frontCoverResult = $database->query(" SELECT b.id, b.filename FROM banners as b WHERE b.keyvalue = '$platform->id' AND b.keytype = 'platform-boxart' ")) {
            if (count($frontCoverResult) > 0) {
                ?>
								<div id="frontBoxartSlider" class="nivoSlider">
							<?php
$frontBoxartSlideCount = 0;
                while ($front = $frontCoverResult->fetch(PDO::FETCH_OBJ)) {
                    ?>
			...
			<div id="fanartScreens">
				...
				<div id="fanart">
					...
					<div class="slider-wrapper theme-default">
						...
						<?php
if ($fanartResult = $database->query(" SELECT b.id, b.filename FROM banners as b WHERE b.keyvalue = '$platform->id' AND b.keytype = 'platform-fanart' ")) {
            $fanSlideCount = 0;
        ...
        ?>
			...
			<div id="banners">
				<div class="slider-wrapper theme-default">
					...
					<?php
if ($bannerResult = $database->query(" SELECT b.id, b.filename FROM banners as b WHERE b.keyvalue = '$platform->id' AND b.keytype = 'platform-banner' ") or die("banner query failed" . mysql_error())) {

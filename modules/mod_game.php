...
if (isset($function) && $function == 'Add Game') {
    ...
    if (count($result) == 0) {
        ...
        if (!empty($id)) {
            $dbGamesResult = $database->query("SELECT `g`.*, `p`.`id` AS `PlatformId`, `p`.`name` AS `PlatformName`, `p`.`alias` AS `PlatformAlias`, `p`.`icon` AS `PlatformIcon` FROM `games` AS `g`, `platforms` AS `p` WHERE `g`.`Platform` = `p`.`id` AND `g`.`id` = $id");
            if ($dbGamesRow = $dbGamesResult->fetch(PDO::FETCH_ASSOC)) {
                $searchParams = array();
        ...
        $existingRow = $result->fetch(PDO::FETCH_OBJ);
        $id = $existingRow->id;
        $tab = 'game';
    }
}
...
if (isset($function) && $function == 'Save Game') {
    ...
    $id = $newshowid;
    if ($result = $database->query($query) or die('Query failed: ' . mysql_error())) {
        $dbGamesResult = $database->query("SELECT `g`.*, `p`.`id` AS `PlatformId`, `p`.`name` AS `PlatformName`, `p`.`alias` AS `PlatformAlias`, `p`.`icon` AS `PlatformIcon` FROM `games` AS `g`, `platforms` AS `p` WHERE `g`.`Platform` = `p`.`id` AND `g`.`id` = $id");
        if ($dbGamesRow = $dbGamesResult->fetch(PDO::FETCH_ASSOC)) {
            $searchParams = array();
            $searchParams['body'] = $dbGamesRow;
...
if (isset($function) && $function == 'Upload Game Banner') {
    ...
    if ($image_width == 760 && $image_height == 140) {
        if ($image_type == '2' || $image_type == '3') {
            ...
            if ($image_type == '2') {
                ## If it's a JPEG name the extesion accordingly
                ...
                ## If it's a PNG name the extesion accordingly
                ## Generate the new filename
                if ($subkey == 'graphical') {
                    if (file_exists("banners/$subkey/$id-g.jpg") || file_exists("banners/$subkey/$id-g.png")) {
                        $filekey = 2;

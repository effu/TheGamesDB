...
if (isset($function) && $function == 'Save Platform') {
    ...
    if ($aliasResult = $database->query(" SELECT p.id FROM platforms AS p WHERE p.alias = '$alias' AND p.id != $platformid ")) {
        if (count($aliasResult) == 0) {
...
if (isset($function) && $function == 'Upload Platform Icon') {
    ...
    if ($errormessage == "") {
        ...
        $prevIconQuery = $database->query(" SELECT icon FROM platforms WHERE id = $platformId LIMIT 1 ");
        $prevIconResults = $prevIconQuery->fetch(PDO::FETCH_OBJ);

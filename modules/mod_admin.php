...
if (isset($function) && $function == "Add New Publisher") {
    if ($adminuserlevel == 'ADMINISTRATOR') {
        ...
        if (empty($_FILES["publisherlogo"]["name"])) {
            if ($database->query("INSERT INTO pubdev (keywords) VALUES ('$publisherKeywords')") or die(mysql_error())) {
            ...
            if ($_FILES['publisherlogo']['error'] == 0) {
                ...
                if (move_uploaded_file($_FILES["publisherlogo"]["tmp_name"], "banners/publisher-logos/$logofilename")) {
                    if ($database->query("INSERT INTO pubdev (keywords, logo) VALUES ('$publisherKeywords', '$logofilename') ") or die(mysql_error())) {
...
if (isset($function) && $function == "Update Publisher") {
    if ($adminuserlevel == 'ADMINISTRATOR') {
        ...
        if (empty($_FILES["publisherlogo"]["name"])) {
            if ($database->query("UPDATE pubdev SET keywords='$publisherKeywords' WHERE id=$publisherID")) {
            ...
            if ($_FILES["publisherlogo"]["error"] > 0) {
                ...
                if (move_uploaded_file($_FILES["publisherlogo"]["tmp_name"], "banners/publisher-logos/$logofilename")) {
                    if ($database->query("UPDATE pubdev SET keywords='$publisherKeywords', logo='$logofilename' WHERE id=$publisherID")) {

...
$totalGamesQuery = $database->query("SELECT `id` from `games`");
$totalGamesCount = count($totalGamesQuery);

// Get Number of Games Having Front Boxart
$frontBoxartQuery = $database->query(" SELECT games.id FROM games, platforms WHERE EXISTS (SELECT keyvalue FROM banners WHERE banners.keyvalue = games.id AND banners.filename LIKE '%front%') AND games.Platform = platforms.id ");
$frontBoxartCount = count($frontBoxartQuery);

// Get Number of Games Having Rear Boxart
$rearBoxartQuery = $database->query(" SELECT games.id FROM games, platforms WHERE EXISTS (SELECT keyvalue FROM banners WHERE banners.keyvalue = games.id AND banners.filename LIKE '%back%') AND games.Platform = platforms.id ");
$rearBoxartCount = count($rearBoxartQuery);

// Get Number of Games Having Fanart
$fanartQuery = $database->query(" SELECT games.id FROM games, platforms WHERE EXISTS (SELECT keyvalue FROM banners WHERE banners.keyvalue = games.id AND banners.keytype = 'fanart') AND games.Platform = platforms.id ");
$fanartCount = count($fanartQuery);

// Get Number of Games Having ClearLOGOs
$clearLogoQuery = $database->query("  SELECT games.id FROM games, platforms WHERE  EXISTS (SELECT keyvalue FROM banners WHERE banners.keyvalue = games.id AND banners.keytype = 'clearlogo') AND games.Platform = platforms.id ");
$clearLogoCount = count($clearLogoQuery);

// Get Number of Games Having Banners
$bannersQuery = $database->query(" SELECT games.id FROM games, platforms WHERE EXISTS (SELECT keyvalue FROM banners WHERE banners.keyvalue = games.id AND banners.keytype = 'series') AND games.Platform = platforms.id ");
$bannersCount = count($bannersQuery);

// Get Number of Games Having Screenshots
$screenshotsQuery = $database->query(" SELECT games.id FROM games, platforms WHERE EXISTS (SELECT keyvalue FROM banners WHERE banners.keyvalue = games.id AND banners.keytype = 'screenshot') AND games.Platform = platforms.id ");
$screenshotsCount = count($screenshotsQuery);

// Get Number of Games Having Overviews
$overviewQuery = $database->query(" SELECT games.id FROM games, platforms WHERE games.Platform = platforms.id AND games.Overview IS NOT NULL ");
$overviewCount = count($overviewQuery);

// Get Number of Games Having Genres
$genresQuery = $database->query(" SELECT games.id FROM games, platforms WHERE games.Platform = platforms.id AND games.Genre IS NOT NULL ");
$genresCount = count($genresQuery);

// Get Number of Games Having Trailers
$trailersQuery = $database->query(" SELECT games.id FROM games, platforms WHERE (games.Youtube IS NOT NULL OR games.Youtube != '') AND games.Platform = platforms.id ");
$trailersCount = count($trailersQuery);

// Get Number of Games Release Date
$releaseDateQuery = $database->query(" SELECT games.id FROM games, platforms WHERE (games.ReleaseDate IS NOT NULL OR games.ReleaseDate != '') AND games.Platform = platforms.id ");
$releaseDateCount = count($releaseDateQuery);

...
$database = new PDO('mysql:host=' . $db_server . ';dbname=' . $db_database, $db_user, $db_password);
$sql = "SET NAMES 'utf8'";
...
function urlseriesname($seriesid)
{
    ...
    $seriesinfo = $result->fetch(PDO::FETCH_OBJ);
...
function bannerdisplay($id)
{
    ...
    $result = $database->query($query);
    while ($banner = $result->fetch(PDO::FETCH_OBJ)) {
        if ($banner->subkey == 'text') {$textbcount++;} else {
...
function cryptemail($myemail)
{
    ...
    do {
        ...
        {print "";} else { $cryptmail = $cryptmail . "&#$code"; // Make a new string with cryptic ASCII codes
        }
        $i++;
        ...
        foreach ($specials as $specialepisode) {
            foreach ($specialepisode as $specialdata) {
                if (key($specialepisode) == $seasonnumber) {
                    ...
                    $result = $database->query($query);
                    $episodes = $result->fetch(PDO::FETCH_OBJ);

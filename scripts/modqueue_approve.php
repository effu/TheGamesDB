...
if ($loggedin = 1 && $adminuserlevel = 'ADMINISTRATOR') {
    if (isset($modID)) {
        ...
        $modItemResult = $database->query("SELECT * from moderation_uploads WHERE id = $modID");
        ...
        if ($deletebanner = mysql_fetch_object($database->query($query))) {
            ## Delete SQL record
        ...
        if (rename("../moderationqueue/$modItem->filename", "../banners/$filename")) {
            ...
            if ($database->query("INSERT INTO banners (keytype, keyvalue, userid, dateadded, filename, languageid, resolution) VALUES ('$keytype', $modItem->gameID, $modItem->userID, $time, '$filename', 1, '$modItem->resolution')")) {

...
if ($loggedin = 1 && $adminuserlevel = 'ADMINISTRATOR') {
    ...
    if (isset($reportID) and isset($reportType) and isset($reportedID)) {
        ...
        if ($database->query("DELETE FROM moderation_reported WHERE id = $reportID")) {
                ...
                if ($reportedResult = $database->query("SELECT filename FROM banners WHERE id = $reportedID LIMIT 1")) {
                    ...
                    if (isset($reported->filename)) {
                        ...
                        if (file_exists("../reportedqueue/_cache/$reported->filename")) {
                            ...
                            // Delete SQL record
                            if ($database->query("DELETE FROM banners WHERE id=$reportedID")) {
                                // Delete ratings
                                $database->query("DELETE FROM ratings WHERE itemtype='banner' AND itemid=$reportedID");
            ...
            if ($reportType == "game") {
                // Delete SQL record
                if ($database->query("DELETE FROM games WHERE id=$reportedID")) {
                    // Delete ratings
                    $database->query("DELETE FROM ratings WHERE itemtype='game' AND itemid=$reportedID");

...
if ($loggedin = 1 && $adminuserlevel = 'ADMINISTRATOR') {
    ...
    if (isset($modimageid)) {
        if ($modimage = mysql_fetch_object($database->query("SELECT * FROM moderation_uploads WHERE id = $modimageid"))) {
            $currentimage = mysql_fetch_object($database->query("SELECT * FROM banners WHERE keyvalue = $modimage->gameID AND filename LIKE '%$modimage->imagekey%' LIMIT 1"));
            ?>

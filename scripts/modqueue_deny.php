...
if ($loggedin = 1 && $adminuserlevel = 'ADMINISTRATOR') {
    if (isset($modID)) {
        ...
        $modItemResult = $database->query("SELECT m.*, g.GameTitle from moderation_uploads AS m, games AS g WHERE m.id = $modID AND g.id = m.gameID");
        $modItem = $modItemResult->fetch(PDO::FETCH_OBJ);

        ## Delete Moderation Item SQL record
        if ($database->query("DELETE FROM moderation_uploads WHERE id=$modItem->id")) {
            ...
            $database->query(" INSERT INTO messages (`from`, `to`, `subject`, `message`, `status`, `timestamp`) VALUES ('$user->id', '$modItem->userID', 'Your submitted artwork for $modItem->GameTitle was denied', '$pmmessage', 'new', FROM_UNIXTIME($time)); ");
            echo "Success";
        } else {

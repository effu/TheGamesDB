...
$result = $database->query($query) or die('Fetch Platform Info Query Failed: ' . mysql_error());
$rows = count($result);
$platform = $result->fetch(PDO::FETCH_OBJ);
?>
...
if (count($result) != 0) {
    ?>
			...
			<div id="gameCoversWrapper">
				<div id="gameCovers">
					<?php
if ($boxartResult = $database->query(" SELECT b.filename FROM banners as b WHERE b.keyvalue = '$platform->id' AND b.keytype = 'platform-boxart' LIMIT 1 ")) {
				...
				<div id="fanartScreens">
					...
					<div id="fanart">
						...
						<div class="slider-wrapper theme-default">
							...
							<?php
if ($fanartResult = $database->query(" SELECT b.filename FROM banners as b WHERE b.keyvalue = '$platform->id' AND b.keytype = 'platform-fanart' ")) {
        $fanSlideCount = 0;
    ...
    ?>
					...
					<div id="screens">
						...
						<div class="slider-wrapper theme-default">
							...
							<?php
if ($screenResult = $database->query(" SELECT b.filename, AVG(r.rating) AS rating FROM games AS g, banners AS b, ratings AS r WHERE r.itemid = b.id AND g.id = b.keyvalue AND r.itemtype = 'banner' AND b.keytype = 'fanart' AND g.Platform = $platform->id GROUP BY b.filename HAVING AVG(r.rating) > 5 ORDER BY AVG(r.rating) DESC LIMIT 10 ")) {
    ...
    ?>
				...
				<div id="banners">
					<div class="slider-wrapper theme-default">
						...
						<?php
if ($bannerResult = $database->query(" SELECT b.filename FROM banners as b WHERE b.keyvalue = '$platform->id' AND b.keytype = 'platform-banner' ") or die("banner query failed" . mysql_error())) {
...
while ($row = $result->fetch(PDO::FETCH_OBJ)) {
            $coverResult = $database->query(" SELECT b.filename FROM banners as b WHERE b.keyvalue = $row->id AND b.keytype = 'boxart' AND b.filename LIKE '%front%' LIMIT 1 ");
            if ($coverResult != false) {
                $cover = $coverResult->fetch(PDO::FETCH_OBJ)

...
if ($function == 'Browse By Platform') {
    ...
    if ($total_pages > 0) {
        if ($searchview != "table") {
            while ($game = $result->fetch(PDO::FETCH_OBJ)) {
...
if ($counter == 2) {
            ...
            ?>
				...
				<?php
while ($game = $result->fetch(PDO::FETCH_OBJ)) {
                if ($gameResult = $database->query(" SELECT g.id, g.GameTitle, g.ReleaseDate, g.Genre, g.Rating, p.name, p.alias AS PlatformAlias, p.icon FROM games as g, platforms as p WHERE g.id = '$game->id' AND g.Platform = p.id")) {

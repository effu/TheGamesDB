...
switch ($StatsType) {
            case "locked":
                ?>
						...
						<?php
$lockedcount = 0;
                $result = $database->query(" SELECT games.id, games.GameTitle, platforms.name, games.lockedby, users.username FROM games, platforms, users WHERE games.Platform = platforms.id AND locked = 'yes' AND games.lockedby = users.id ORDER BY games.GameTitle ASC");
...
break;
            ...
            case "missingplatform":
                ?>
						...
						<?php
$missingcount = 0;
                $result = $database->query(" SELECT games.id, games.GameTitle FROM games WHERE games.Platform IS NULL ORDER BY games.GameTitle ASC ");
                while ($row = $result->fetch(PDO::FETCH_ASSOC)) {
                    ?>
							<tr>
								<td><?php echo $row[id]; ?></td>
...
break;
            ...
            case "multipleplatform":
                ?>
						...
						<?php
$multiplecount = 0;
                $result = $database->query(" SELECT games.id, games.GameTitle, platforms.name FROM games, platforms WHERE games.Platform = platforms.id AND games.Platform LIKE '|%|%|' ORDER BY games.GameTitle ASC ");
                while ($row = $result->fetch(PDO::FETCH_ASSOC)) {
                    ?>
							<tr>
								<td><?php echo $row[id]; ?></td>
...
break;
            ...
            case "morefront":
                ?>
						...
						<?php
$morecount = 0;
                $result = $database->query(" SELECT games.id, games.GameTitle, platforms.name FROM games, platforms WHERE (SELECT COUNT(filename) FROM banners WHERE banners.keyvalue = games.id AND banners.filename LIKE '%front%') > 1 AND games.Platform = platforms.id ORDER BY games.GameTitle ASC ");
                while ($row = $result->fetch(PDO::FETCH_ASSOC)) {
                    ?>
							<tr>
								<td><?php echo $row[id]; ?></td>
            ...
            case "newgametitles":
                ...
                $newGamesResult = $database->query("
                            ...
                            ORDER BY
                                g.created DESC
                        ");
                $newGamesCount = count($newGamesResult);
                ?>
                        <h2 style="color: #FF4F00;">New Games Titles (<?=$newGamesCount?>)</h2>
                            ...
                            <table align="center" border="1" cellspacing="0" cellpadding="7">
								...
								<?php
while ($game = $newGamesResult->fetch(PDO::FETCH_OBJ)) {
            ...
            while ($row = $result->fetch(PDO::FETCH_ASSOC)) {
                ?>
								<tr>
									...
									<td><?php echo $row[name]; ?></td>
									<td><?php echo $row[toprating]; ?></td>
									<td><?php imageResize("banners/" . $row[filename], "banners/_statscache/" . $row[filename], 150);
                echo "<a href=\"$baseurl/banners/$row[filename]\" target=\"_blank\"><img src=\"$baseurl/banners/_statscache/$row[filename]\" alt=\"$row[GameTitle] Fanart\" /></a>";?></td>
								</tr>
								<?php
$missingcount++;
            }

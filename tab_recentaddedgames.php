...
?>
	...
	<?php
$recentResult = $database->query(" SELECT g.*, p.name, p.icon, p.alias AS PlatformAlias FROM games AS g, platforms AS p WHERE g.Platform = p.id AND g.created IS NOT NULL ORDER BY g.created DESC LIMIT 50 ");
$count = 1;
var_dump($recentResult);

	...
	<?php
        ...
        if ($favoritesview != "table") {
            foreach ($favoritesArray as $favoriteID) {
                if ($gameResult = $database->query(" SELECT g.id, g.GameTitle, g.Overview, p.name, p.alias AS PlatformAlias, p.icon FROM games as g, platforms as p WHERE g.id = '$favoriteID' AND g.Platform = p.id")) {
                    if ($game = $gameResult->fetch(PDO::FETCH_OBJ)) {
...
$boxartQuery = $database->query("SELECT keyvalue FROM banners WHERE banners.keyvalue = '$game->id' AND banners.filename LIKE '%front%' LIMIT 1");
						...
						<?php
                            ...
                            if ($boxartResult = $database->query(" SELECT b.filename FROM banners as b WHERE b.keyvalue = '$favoriteID' AND b.filename LIKE '%boxart%front%' LIMIT 1 ")) {
                                $boxart = $boxartResult->fetch(PDO::FETCH_OBJ);
                            }
                            ?>
...
if ($boxart->filename != "") {
                            ...
                            ?>
									...
									<?php
$platformIdQuery = $database->query("SELECT * FROM platforms WHERE id = '$game->Platform' LIMIT 1");
                            $platformIdResult = $platformIdQuery->fetch(PDO::FETCH_OBJ);
...
if ($counter == 2) {
                            ...
                            if ($bannerResult = $database->query(" SELECT b.filename FROM banners as b WHERE b.keyvalue = '$favoriteID' AND b.keytype = 'series' LIMIT 1 ")) {
...
if ($counter == 2) {
			...
			<?php
foreach ($favoritesArray as $favoriteID) {
                if ($gameResult = $database->query(" SELECT g.id, g.GameTitle, g.Genre, g.Rating, p.name, p.alias AS PlatformAlias, p.icon FROM games as g, platforms as p WHERE g.id = '$favoriteID' AND g.Platform = p.id")) {
                    if ($game = $gameResult->fetch(PDO::FETCH_OBJ)) {
                        $boxartQuery = $database->query("SELECT keyvalue FROM banners WHERE banners.keyvalue = '$game->id' AND banners.filename LIKE '%front%' LIMIT 1");
                        $boxartResult = count($boxartQuery);

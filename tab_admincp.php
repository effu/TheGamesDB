...
// Fetch number of uploads to be moderated
    $modQueueResult = $database->query("SELECT id FROM moderation_uploads");
    $modQueueCount = count($modQueueResult);

    // Fetch number of reported images to be moderated
    $repQueueResult = $database->query("SELECT id FROM moderation_reported");
    $repQueueCount = count($repQueueResult);
    ?>
...
## Display language selector
    ...
    break;
            ...
            case "moderationqueue":
                ?>
											...
											<?php
    // Fetch number of uploads to be moderated
                $frontQueueResult = $database->query("SELECT id FROM moderation_uploads WHERE imagekey = 'front'");
                $frontQueueCount = count($frontQueueResult);
                if (empty($frontQueueCount)) {$frontQueueCount = 0;}

                $backQueueResult = $database->query("SELECT id FROM moderation_uploads WHERE imagekey = 'back'");
                $backQueueCount = count($backQueueResult);
                if (empty($backQueueCount)) {$backQueueCount = 0;}

                $clearlogoQueueResult = $database->query("SELECT id FROM moderation_uploads WHERE imagekey = 'clearlogo'");
                $clearlogoQueueCount = count($clearlogoQueueResult);
    ...
    if ($queuetype == "frontboxart") {
                    $modQueueResult = $database->query("SELECT m.*, u.username, g.GameTitle, p. NAME AS PlatformName, p.id AS PlatformID FROM moderation_uploads AS m, users AS u, games AS g, platforms AS p WHERE imagekey = 'front' AND m.userID = u.id AND m.gameID = g.id AND g.Platform = p.id ORDER BY dateadded");
                    $queueheader = "Front Boxart Moderation Queue";
                } else if ($queuetype == "backboxart") {
                $modQueueResult = $database->query("SELECT m.*, u.username, g.GameTitle, p. NAME AS PlatformName, p.id AS PlatformID FROM moderation_uploads AS m, users AS u, games AS g, platforms AS p WHERE imagekey = 'back' AND m.userID = u.id AND m.gameID = g.id AND g.Platform = p.id ORDER BY dateadded");
                $queueheader = "Rear Boxart Moderation Queue";
            } else if ($queuetype == "clearlogo") {
                $modQueueResult = $database->query("SELECT m.*, u.username, g.GameTitle, p. NAME AS PlatformName, p.id AS PlatformID FROM moderation_uploads AS m, users AS u, games AS g, platforms AS p WHERE imagekey = 'clearlogo' AND m.userID = u.id AND m.gameID = g.id AND g.Platform = p.id ORDER BY dateadded");
            ...
            ?>
								...
								<table call-padding="0" cell-spacing="0" style="border: 2px solid #444; border-radius: 6px; background-color: #EEEEEE; color: #333333; border-collapse: separate; border-spacing: 2px; border-color: gray; width: 100%;">
									...
									<tbody>
										<?php
if ($modQueueCount > 0) {
                while ($modQueueObject = $modQueueResult->fetch(PDO::FETCH_OBJ)) {
                    ?>
...
// Fetch number of reported images to be moderated
            ...
            //Front Boxart
            $reportedFrontQueueResult = $database->query("SELECT m.id FROM moderation_reported AS m, banners AS b WHERE m.reportid = b.id AND m.reporttype = 'image' AND b.keytype = 'boxart' AND b.filename LIKE '%front%'");
            $reportedFrontQueueCount = count($reportedFrontQueueResult);

            //Rear Boxart
            $reportedRearQueueResult = $database->query("SELECT m.id FROM moderation_reported AS m, banners AS b WHERE m.reportid = b.id AND m.reporttype = 'image' AND b.keytype = 'boxart' AND b.filename LIKE '%back%'");
            $reportedRearQueueCount = count($reportedRearQueueResult);

            //ClearLOGO
            $reportedClearlogoQueueResult = $database->query("SELECT m.id FROM moderation_reported AS m, banners AS b WHERE m.reportid = b.id AND m.reporttype = 'image' AND b.keytype = 'clearlogo'");
            $reportedClearlogoQueueCount = count($reportedClearlogoQueueResult);

            //Fanart
            $reportedFanartQueueResult = $database->query("SELECT m.id FROM moderation_reported AS m, banners AS b WHERE m.reportid = b.id AND m.reporttype = 'image' AND b.keytype = 'fanart'");
            $reportedFanartQueueCount = count($reportedFanartQueueResult);

            //Screenshot
            $reportedScreenshotQueueResult = $database->query("SELECT m.id FROM moderation_reported AS m, banners AS b WHERE m.reportid = b.id AND m.reporttype = 'image' AND b.keytype = 'screenshot'");
            $reportedScreenshotQueueCount = count($reportedScreenshotQueueResult);

            //Banner
            $reportedBannerQueueResult = $database->query("SELECT m.id FROM moderation_reported AS m, banners AS b WHERE m.reportid = b.id AND m.reporttype = 'image' AND b.keytype = 'series'");
            $reportedBannerQueueCount = count($reportedBannerQueueResult);

            //Games
            $reportedGameQueueResult = $database->query("SELECT m.id FROM moderation_reported AS m, games AS g WHERE m.reportid = g.id AND m.reporttype = 'game'");
            $reportedGameQueueCount = count($reportedGameQueueResult);
            ?>
...
switch ($queuetype) {
                ...
                case "game":
                    $reportedResult = $database->query("SELECT m.*, u.username, g.id AS gameID, g.GameTitle, g.Developer, g.Publisher, g.ReleaseDate, g.Overview, p.name AS PlatformName, p.id AS PlatformID FROM moderation_reported AS m, users AS u, games AS g, platforms AS p WHERE m.reportid = g.id AND m.reporttype = 'game' AND m.userID = u.id AND g.Platform = p.id ORDER BY m.dateadded") or die(mysql_error());
                    $queueheader = "Reported Game Queue";
                    break;
            }

            $reportedCount = count($reportedResult);
            ?>
								...
								<table call-padding="0" cell-spacing="0" style="border: 2px solid #444; border-radius: 6px; background-color: #EEEEEE; color: #333333; border-collapse: separate; border-spacing: 2px; border-color: gray; width: 100%;">
									...
									<tbody>
										<?php
if ($reportedCount > 0) {
                while ($reportedObject = $reportedResult->fetch(PDO::FETCH_OBJ)) {
                    ?>
...
break;
        ...
        case "pubdev":
            ?>
								...
								<?php
$pubdevQuery = $database->query(" SELECT * FROM pubdev ORDER BY keywords ASC");
            while ($pubdevResult = $pubdevQuery->fetch(PDO::FETCH_OBJ)) {
                ?>
									<tr>
            ...
            ?>
							...
							<div id="pm-userlist" style="display: none;">
								<div style="height: 400px; overflow: auto;">
									<p>
								<?php
$userlistQuery = $database->query("SELECT id, username FROM users ORDER BY username ASC");
            while ($userlist = $userlistQuery->fetch(PDO::FETCH_OBJ)) {
                ?>
        ...
        case "platformalias":
            ?>
							...
							<table align="center" border="1" cellspacing="0" cellpadding="7" bgcolor="#888888">
								...
								<?php
$platformsResult = $database->query(" SELECT p.id, p.name, p.alias FROM platforms AS p ORDER BY p.id ");
            while ($platforms = $platformsResult->fetch(PDO::FETCH_OBJ)) {
...
// Main Switchboard
            switch ($command) {
                ...
                case ('populategames'):
                    ...
                    $dbGamesResult = $database->query("SELECT `g`.*, `p`.`id` AS `PlatformId`, `p`.`name` AS `PlatformName`, `p`.`alias` AS `PlatformAlias`, `p`.`icon` AS `PlatformIcon` FROM `games` AS `g`, `platforms` AS `p` WHERE `g`.`Platform` = `p`.`id`");
                    while ($dbGamesRow = $dbGamesResult->fetch(PDO::FETCH_ASSOC)) {
                        $searchParams = array();

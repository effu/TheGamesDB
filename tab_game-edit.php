...
$platformQuery = $database->query(" SELECT * FROM platforms ORDER BY name ASC");
        ?>
				...
				<select name="Platform"<?php if ($adminuserlevel != 'ADMINISTRATOR') {echo "disabled";}?>>
					<?php
while ($platformResult = $platformQuery->fetch(PDO::FETCH_OBJ)) {
            ?>
					...
					<tr>
						...
						<td>
							<select name="Players">
								...
								<option value="1" <?php if ($game->Players == 1) {
            echo 'selected';
        }
        ?>>1</option>
								<option value="2" <?php if ($game->Players == 2) {
            echo 'selected';
        }
        ?>>2</option>
								<option value="3" <?php if ($game->Players == 3) {
            echo 'selected';
        }
        ?>>3</option>
								<option value="4" <?php if ($game->Players == 4) {
            echo 'selected';
        }
        ?>>4+</option>
									?>
							</select>&nbsp;&nbsp;&nbsp;&nbsp;<span class="grey">Co-op:</span><input type="checkbox" id="coopfake" <?php if ($game->coop == "Yes") {
            echo 'checked';
        }
        ?> />
					...
					<tr>
						...
						<td>
							<select name="Rating">
								...
								<option <?php if ($game->Rating == 'EC - Early Childhood') {
            print 'selected';
        }
        ?>>EC - Early Childhood</option>
								<option <?php if ($game->Rating == 'E - Everyone') {
            print 'selected';
        }
        ?>>E - Everyone</option>
								<option <?php if ($game->Rating == 'E10+ - Everyone 10+') {
            print 'selected';
        }
        ?>>E10+ - Everyone 10+</option>
								<option <?php if ($game->Rating == 'T - Teen') {
            print 'selected';
        }
        ?>>T - Teen</option>
								<option <?php if ($game->Rating == 'M - Mature') {
            print 'selected';
        }
        ?>>M - Mature</option>
								<option <?php if ($game->Rating == 'RP - Rating Pending') {
            print 'selected';
        }
        ?>>RP - Rating Pending</option>
							</select>
						</td>
					</tr>
					<tr>
...
if ($bannerResult = $database->query(" SELECT b.id, b.filename FROM banners as b WHERE b.keyvalue = '$game->id' AND b.keytype = 'series' ") or die("banner query failed" . mysql_error())) {
            if (count($bannerResult) > 0) {
                ?>
						...
						<?php
$bannerSlideCount = 0;
                while ($banner = $bannerResult->fetch(PDO::FETCH_OBJ)) {
                    ?>
